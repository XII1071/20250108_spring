...
public interface JournalRepository extends JpaRepository<Journal, Long> {
  ...
  @Query("select j, p, avg(coalesce(c.likes,0)), count(distinct c) " +
      "from Journal j " +
  ...
  @Query("select j, p, avg(coalesce(c.likes, 0)), count(c) " +
      "from Journal j left outer join Photos p on p.journal=j " +
      "left outer join Comments c on c.journal = j " +
      "where j.jno = :jno group by p ")
  List<Object[]> getJournalWithAll(@Param("mid") Long jno); //특정 영화 조회
}
